...
public class HomePageDataServcieImpl implements HomePageDataServcie {
	...
	public FindExclusiveCouponResponse findExclusiveCoupon(String userId, FindExclusiveCouponResponse response) {
		...
		AdGroupActivityConn adGroupActivityConn = adAppHomePageDao.findAdGroupActivityConnByActivityIdAndUserId(exclusiveCouponActivityId, userId);
		if (adGroupActivityConn == null) {
			...
			// 2.查询专属优惠券有哪些
			List<ExclusiveCoupon> exclusiveCouponList = adAppHomePageDao.findExclusiveCoupon(exclusiveCouponActivityId, exclusiveCouponCode);
			...
			if (exclusiveCouponList.size() > 0) {
				...
				// 3.查询用户领取了哪些专属优惠券
				List<AdCouponCode> AdCouponCodeList = findUserReceivedExclusiveCoupon(userId, couponIds, activityId, exclusiveCouponList);
	 ...
	 * 查询用户，领取过哪些专属优惠券
	 */
	private List<AdCouponCode> findUserReceivedExclusiveCoupon(String userId, List<Long> couponIds, Long activityId, List<ExclusiveCoupon> exclusiveCouponList) {
	 ...
	 */
	private List<ExclusiveCoupon> processingExclusiveCouponList(List<ExclusiveCoupon> exclusiveCouponList, List<Long> couponIds) {
		for (ExclusiveCoupon exclusiveCoupon : exclusiveCouponList) {
	...
	@Override
	public ReceiveExclusiveCouponResponse receiveExclusiveCoupon(ReceiveExclusiveCouponRequest request, ReceiveExclusiveCouponResponse response) {
		...
		for (String couponId : couponIdList) {
			...
			messageDataBean = freeCouponBusinessServiceI.
					receiveCoupon(receiveExclusiveCoupon.getUserId(),
							receiveExclusiveCoupon.getCouponId(),
							receiveExclusiveCoupon.getActivityId(),
							null);
	...
	@Override
	public GetAdCouponActivityInfosResponse getAdCouponActivityInfos(String activityId, GetAdCouponActivityInfosResponse response) {
