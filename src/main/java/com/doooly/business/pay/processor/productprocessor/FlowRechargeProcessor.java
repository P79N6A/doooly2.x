 ...
 */
@Component
public class FlowRechargeProcessor implements ProductProcessor{

	protected Logger logger = LoggerFactory.getLogger(FlowRechargeProcessor.class);
	...
	public int getProcessCode() {
		return ProductType.FLOW_RECHARGE.getCode();
	}
	...
	public PayMsg process(OrderVo order) {
		if(order != null && order.getItems() !=null){
			...
			if (!OfPayService.OF_SUCCESS_CODE.equals(oldItem.getRetState())) {
				Map<String, String> retMap = ofPayService.flowRecharge(order);
	...
	private int updateOrderItemSuccesss(OrderItemVo oldItem, Map<String, String> retMap) {
		try {
			...
			newItem.setUpdateBy("FlowRechargeProcessor");
