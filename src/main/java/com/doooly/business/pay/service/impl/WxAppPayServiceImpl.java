...
public class WxAppPayServiceImpl extends AbstractPaymentService {
	...
	// 微信支付参数
	public final static String WXAPP_APPID = PropertiesHolder.getProperty("wx.appid");
	public final static String WXAPP_MCH_ID = PropertiesHolder.getProperty("wx.mch_id");
	public final static String WXAPP_TRADE_TYPE = PropertiesHolder.getProperty("wx.trade_type");
	public final static String WXAPP_KEY = PropertiesHolder.getProperty("wx.key");
	public final static String WXAPP_UNIFIEDORDER_URL = PropertiesHolder.getProperty("wx.unifiedorder_url");
	public final static String WXAPP_NOTIFY_URL = PropertiesHolder.getProperty("wx.notify_url");
	public final static String WXAPP_ORDERQUERY_URL = PropertiesHolder.getProperty("wx.orderquery_url");
	...
	protected Map<String, Object> resolveAndVerifyResult(String retStr, String payType) {
		try {
			...
			boolean bool = WxUtil.verify(sortMap, result.getSign(), WXAPP_KEY);
			if (!bool) {
	...
	private String orderQuery(WxQueryParams data) {
		try {
			...
			String sign = WxUtil.createSign(params, WXAPP_KEY);
			data.setSign(sign);
			String xmlParas = WxUtil.toXml(data, WxQueryParams.class);
			logger.info("orderQuery() xmlParas = {}", xmlParas);
			String retXml = HttpClientUtil.httpsRequest(WXAPP_ORDERQUERY_URL, xmlParas);
	...
	private WxQueryParams buildQueryParams(PayFlow flow) {
		...
		params.setAppid(WXAPP_APPID);
		params.setMch_id(WXAPP_MCH_ID);
	...
	private SortedMap<String, Object> getPayForm(WxPrePayResult retVo) {
		...
		String paySign = WxUtil.createSign(signMap, WXAPP_KEY);
	...
	private WxPrePayParams prePayParams(String orderNum,BigDecimal totalAmount,String goodsDesc,String clientIp,PayFlow payFlow)  {
		...
		//应用ID
		data.setAppid(WXAPP_APPID); 
		//商户号
		data.setMch_id(WXAPP_MCH_ID);
		//随机字符串	
		...
		//通知地址
		data.setNotify_url(WXAPP_NOTIFY_URL);
		//交易类型
		data.setTrade_type(WXAPP_TRADE_TYPE);
		return data;
	}	
	...
	private WxPrePayResult unifiedOrder(WxPrePayParams data) {
		try {
			...
			String sign = WxUtil.createSign(parameters, WXAPP_KEY);
			data.setSign(sign);
			//生成微信预付单
			String prePayXml = WxUtil.toXml(data,WxPrePayParams.class);
			logger.info("unifiedOrder() xml = {}", prePayXml);
			String retXml = HttpClientUtil.httpsRequest(WXAPP_UNIFIEDORDER_URL, prePayXml);
