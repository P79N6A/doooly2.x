...
public class ProductServicePublish {
	...
	public String nexusProductInfo(JSONObject params) {
		AdSelfProduct mobikePro = productService.getProductSku(Integer.valueOf(NEXUS_MERCHANT_ID),
				Integer.valueOf(NEXUS_PRODUCT_ID), null);
	...
	public String mobikeProductInfo(JSONObject params) {
		AdSelfProduct mobikePro = productService.getProductSku(Integer.valueOf(MOBIKE_MERCHANT_ID),
				Integer.valueOf(MOBIKE_PRODUCT_ID), null);
		JSONObject retJson = new JSONObject(true);
		List<AdSelfProductSku> mobileList = mobikePro.getProductSku();
		String default_sku_id = "";
		JSONArray array = new JSONArray();
	...
	public String virProducts(JSONObject params) {
		...
		try {
			...
			JSONObject retJson = new JSONObject(true);
			// 查询可用积分
			BigDecimal availablePoint = adUserDao.getAvailablePoint(params.getString("userId"));
			// 查询待返积分
			retJson.put("availablePoint", availablePoint != null ? availablePoint : new BigDecimal("0"));
			
			// ==================== 话费商品信息 ====================================
			AdSelfProduct mobilePro = productService.getProductSku(Integer.valueOf(MERCHANT_ID),
					Integer.valueOf(CMCC_TEL_ID), null);
			...
			AdSelfProduct flowPro = productService.getProductSku(Integer.valueOf(MERCHANT_ID),
					Integer.valueOf(CMCC_FLOW_ID), null);
			array = new JSONArray();
			...
			// ==================== 旅游卡商品信息 ====================
			AdSelfProduct sctcdPro = productService.getProductSku(Integer.valueOf(SCTCD_MERCHANT_ID),
					Integer.valueOf(SCTCD_PRODUCT_ID), null);
			...
			if (conf != null) {
				...
				if (conf.getDiscountsStartDate() != null && conf.getDiscountsStartDate() != null) {
					...
					if (conf.getDiscountsStartDate().getTime() <= t && t <= conf.getDiscountsEndDate().getTime()) {
						...
						retJson.put("discountsStartDate",
								DateUtils.formatDate(conf.getDiscountsStartDate(), "yyyy.MM.dd"));
